...
class PaginatorHelper extends Helper
{
    ...
    /**
     * Cached paging data merged from request + helper options.
     *
     * @var array
     */
    protected $_paging = [];
    ...
    public function __construct(View $View, array $config = [])
    {
        ...
        $this->_paging = (array)$request->getAttribute('paging', $request->getParam('paging', []));

        $query = $request->getQueryParams();
        unset($query['page'], $query['limit'], $query['sort'], $query['direction']);
        $this->setConfig(
            'options.url',
            array_merge((array)$request->getParam('pass', []), ['?' => $query])
        );
    }
    ...
    {
        $paging = $this->_paging ?: (array)$this->getView()->getRequest()->getAttribute('paging', $this->getView()->getRequest()->getParam('paging', []));
    ...
    public function options(array $options = [])
    {
        if (!empty($options['paging'])) {
            $this->_paging = $options['paging'] + $this->_paging;
            unset($options['paging']);
        }
        $model = $this->defaultModel();

        if (!empty($options[$model])) {
            if (!isset($this->_paging[$model])) {
                $this->_paging[$model] = [];
            }
            $this->_paging[$model] = $options[$model] + $this->_paging[$model];
            unset($options[$model]);
        }
    ...
    public function generateUrl(array $options = [], $model = null, $full = false)
    {
        ...
        if (isset($paging['sortDefault'], $paging['directionDefault'], $url['sort'], $url['direction']) &&
            ...
            unset($url['sort'], $url['direction']);
        }
        $buildOptions = $full ? ['fullBase' => true] : [];
        return $this->Url->build($url, $buildOptions);
    }
    ...
    {
        $params = $this->params($model);
        return !empty($params) && !empty($params[$page . 'Page']);
    }
    ...
    public function defaultModel()
    {
        ...
        $paging = $this->_paging ?: (array)$this->getView()->getRequest()->getParam('paging', []);
        if (empty($paging)) {
            return null;
        }
        list($this->_defaultModel) = array_keys($paging);
        return $this->_defaultModel;
    }
     ...
     *
     * @param string $ellipsis Ellipsis markup/text.
     * @param array $params Params from the numbers() method.
